        ...
        <Styled {...rest}>
            <li>
                <NavLink className={'menu__link '} to={`/employee/activity-log/${id}`}><span
                    ...
                    src={logsIcon} alt=""/>
                    <span>{t('Журнал активности')}</span></span> <span
                    className={'count'}>{get(counts, 'activity', 0)}</span></NavLink>
            </li>
            <li>
                <NavLink className={'menu__link '} to={`/employee/keylogger/${id}`}><span
                    className={"menu__link__left"}><img src={keyloggerIcon} alt=""/>
                    <span>{t('Кейлогер')}</span></span> <span
                    className={'count'}>{get(counts, 'keylogger', 0)}</span></NavLink>
            </li>
            <li>
                <NavLink className={'menu__link'} to={`/employee/screenshot/${id}`}><span
                    className={"menu__link__left"}><img src={screenIcon} alt=""/>
                    <span>{t('Скриншоты')}</span></span> <span className={'count'}>{get(counts, 'screenshot', 0)}</span>
                </NavLink>
            </li>
            <li>
            ...
            <li>
                <NavLink className={'menu__link'} to={`/employee/buffer/${id}`}><span
                    ...
                    <span>{t('Буфер')}</span></span> <span
                    className={'count'}>{get(counts, 'clipboard', 0)}</span></NavLink>
            </li>
            <li>
            ...
            <li>
                <NavLink className={'menu__link'} to={`/employee/telegram/${id}`}><span
                    ...
                    <span>{t('Телеграм')}</span></span> <span
                    className={'count'}>{get(counts, 'telegram', 0)}</span></NavLink>
            </li>
            <li>
                <NavLink className={'menu__link'} to={`/employee/usb-devices/${id}`}><span
                    className={"menu__link__left"}><img src={usbIcon} alt=""/>
                    <span>{t('USB лист')}</span></span> <span
